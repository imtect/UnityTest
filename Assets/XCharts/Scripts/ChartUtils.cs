...
namespace xcharts {
    public static class ChartUtils {
        ...
        public static Vector2 MagicNumbers(float max, float min, bool interger = false) {
            ...
            if (diff != 0)
                digit = (int)Mathf.Log10(diff) + 1;
            else
                digit = 1;

            int temp = (int)(maxInt % (Mathf.Pow(10, digit)) / (Mathf.Pow(10, digit - 1)));

            int maxY = (int)(maxInt / Mathf.Pow(10, digit)) * (int)Mathf.Pow(10, digit) + (temp % 2 == 0 ? temp + 2 : temp + 1) * (int)Mathf.Pow(10, digit - 1);

            int diff2 = maxY - minInt;

            int digit2 = (int)Mathf.Log10(diff2) + 1;

            int temp2 = (int)(diff2 / Mathf.Pow(10, digit2 - 1));

            int diffMax;

            diffMax = (temp2 % 2 == 0 ? temp2 + 2 : temp2 + 1) * (int)Mathf.Pow(10, digit2 - 1);

            int minY = maxY - diffMax;
            ...
            if (interger) {
                ...
                if (diffU < 20) {
                    int digitU = (int)Mathf.Log10(diffU) + 1;
                    int tempU = (int)(diffU / Mathf.Pow(10, digitU - 1));

                    if (tempU == 2 || tempU == 6) {
                        maxY = maxY + 2 * (int)Mathf.Pow(10, digitU - 1);
                    }
                }
            }
